...
public class LinkConeLightSystem implements EntitySystem {
    ...
    public void update(Engine engine) {
        for (final var light : engine.environment().fetchAll(LIGHTS)) {
            light.get(ConeLightComponent.class).direction = light.get(FlexLinkComponent.class).angle.addDegrees(180);
        }
        for (final var glow : engine.environment().fetchAll(GLOWS)) {
            glow.get(ConeGlowComponent.class).direction = glow.get(FlexLinkComponent.class).angle.addDegrees(180);
        }
    }
}
