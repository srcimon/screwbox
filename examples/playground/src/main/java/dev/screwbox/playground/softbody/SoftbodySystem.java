...
public class SoftbodySystem implements EntitySystem {
    private static final Archetype SOFTBODIES = Archetype.ofSpacial(SoftbodyComponent.class, ElasticLinkComponent.class);
    ...
    private static void extracted(final Environment environment, final Entity softbody, final List<Entity> nodes) {
        ...
        var joint = softbody.get(ElasticLinkComponent.class);
        boolean done = false;
        nodes.add(softbody);
        while (nonNull(joint) && !done) {
            ...
            if (!closed.contains(targetEntity)) {
                ...
                joint = targetEntity.get(ElasticLinkComponent.class);
