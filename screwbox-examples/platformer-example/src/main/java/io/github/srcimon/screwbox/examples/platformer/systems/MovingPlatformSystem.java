...
public class MovingPlatformSystem implements EntitySystem {
    ...
    public void update(Engine engine) {
        for (var platform : engine.ecosphere().fetchAll(PLATFORMS)) {
    ...
    private void movePlattform(Entity platform, Engine engine) {
        ...
        if (isNull(plattformComponent.targetPosition)) {
            Entity tartetEntity = engine.ecosphere().forcedFetchById(plattformComponent.waypoint);
        ...
        if (distance.isZero()) {
            Entity currentTarget = engine.ecosphere().forcedFetchById(plattformComponent.waypoint);
            Entity nextTarget = engine.ecosphere()
