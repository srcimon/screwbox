package io.github.simonbas.screwbox.core.mouse.internal;
...
class DefaultMouseTest {
    ...
    void worldPosition_returnsTheLatestWorldPosition() {
        ...
        when(graphics.screen()).thenReturn(screen);
        when(screen.position()).thenReturn(Offset.at(40, 12));
        when(graphics.worldPositionOf(Offset.at(111, 230))).thenReturn(Vector.$(40, 90));
        when(graphics.worldPositionOf(Offset.at(179, 8))).thenReturn(Vector.$(10, 30));
        mouse.mouseMoved(mouseEvent);
    ...
    void drag_noMovement_isZero() {
        when(graphics.worldPositionOf(Offset.origin())).thenReturn(Vector.$(40, 90));
