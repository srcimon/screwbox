...
class RenderSystemTest {
    ...
    @Test
    void update_sortOrthographic_drawsNoSpriteUsingZIndex(DefaultEnvironment environment) {
        var sprite = SpriteBundle.ICON.get();
        when(camera.zoom()).thenReturn(2.0);
        when(viewport.toCanvas($$(184, 184, 32, 32), 1, 1)).thenReturn(new ScreenBounds(20, 20, 8, 8));
        when(canvas.size()).thenReturn(Size.of(800, 800));
        ...
        environment
                .addEntity(new Entity()
                        ...
                        .add(new RenderComponent(sprite, 5), render -> render.isSortOrthographic = true))
                .addSystem(new RenderSystem());

        environment.update();

        verify(canvas).drawSprite(sprite, Offset.at(20, 20), SpriteDrawOptions.scaled(2).drawOrder(5).zIndex(28));
    }
}
