...
public class PhysicsDebugSystem implements EntitySystem {
    ...
    public void update(final Engine engine) {
        for (final var entity : engine.ecosphere().fetchAll(COLLIDERS)) {
            renderEntity(engine, entity, Color.BLUE);
        }

        for (final var entity : engine.ecosphere().fetchAll(PHYSICS)) {
            renderEntity(engine, entity, Color.RED);
        }
        for (final var entity : engine.ecosphere().fetchAll(SENSORS)) {
