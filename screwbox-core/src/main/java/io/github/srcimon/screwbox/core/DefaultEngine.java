...
import io.github.srcimon.screwbox.core.audio.internal.PlaybackTracker;
...
class DefaultEngine implements Engine {
    ...
    DefaultEngine(final String name) {
        ...
        final PlaybackTracker playbackTracker = new PlaybackTracker(audioAdapter, dataLinePool);
        final VolumeMonitor volumeMonitor = new VolumeMonitor(executor, audioAdapter, audioConfiguration);
        audio = new DefaultAudio(executor, audioConfiguration, volumeMonitor, camera, playbackTracker);
