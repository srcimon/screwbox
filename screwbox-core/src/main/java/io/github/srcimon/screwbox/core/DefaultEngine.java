...
import io.github.srcimon.screwbox.core.graphics.internal.ViewportSupport;
...
class DefaultEngine implements Engine {
    ...
    DefaultEngine(final String name) {
        ...
        final ViewportSupport viewportSupport = new ViewportSupport(frame, standbyProxyRender, graphicsConfiguration, screen);
        graphics = new DefaultGraphics(graphicsConfiguration, screen, world, light, graphicsDevice, camera, asyncRenderer, viewportSupport);
        ui = new DefaultUi(this, scenes);
        keyboard = new DefaultKeyboard();
        mouse = new DefaultMouse(screen, world);
        loop = new DefaultLoop(List.of(keyboard, viewportSupport, graphics, scenes, ui, mouse, window, camera, particles, audio));
