...
public record SmokeOptions(double viscosity, double diffusion, int iterations, Percent opacity, Percent fade, Vector baseVelocity, Percent baseVelocityAdaption) {

    //TODO add validations
    public static SmokeOptions vaporPreset() {//TODO find better names and presets
        return new SmokeOptions(0.0000000004, 0.000001, 2, Percent.max(), Percent.of(0.04), null, Percent.of(0.001));
    }

    public SmokeOptions diffusion(final double diffusion) {
        return new SmokeOptions(viscosity, diffusion, iterations, opacity, fade, baseVelocity, baseVelocityAdaption);
    }
