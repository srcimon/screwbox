package dev.screwbox.core.graphics.postprocessing.internal;
...
public class ShockwavePostFilter implements PostProcessingFilter {


    private final List<Shockwave> waves;
    private final int tileSize;

    public ShockwavePostFilter(final List<Shockwave> waves, final int cellSize) {
        ...
        for (int y = 0; y < context.height(); y += tileSize) {
            for (int x = 0; x < context.width(); x += tileSize) {
                ...
                for (final var wave : calculatedWaves) {
                    final Offset dist = absolute.subtract(wave.position);
