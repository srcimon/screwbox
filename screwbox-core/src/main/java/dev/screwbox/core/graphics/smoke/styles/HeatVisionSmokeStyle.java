...
public class HeatVisionSmokeStyle implements SmokeStyle {
    ...
    public int apply(float red, float green, float blue, float alpha) {
        float funR = (float) Math.sin(alpha * 2.0 * Math.PI + 0.0) * 0.5f + 0.5f;
        float funG = (float) Math.sin(alpha * 2.0 * Math.PI + 2.094) * 0.5f + 0.5f;
        float funB = (float) Math.sin(alpha * 2.0 * Math.PI + 4.188) * 0.5f + 0.5f;
